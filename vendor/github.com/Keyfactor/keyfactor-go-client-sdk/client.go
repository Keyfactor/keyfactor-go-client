...
func (c *APIClient) prepareRequest(
	...
	if ctx != nil {
		...
		envCtx := context.WithValue(ctx, ContextBasicAuth, BasicAuth{
			UserName: os.Getenv(EnvCommandUsername),
			Password: os.Getenv(EnvCommandPassword),
		})

		// Basic HTTP Authentication
		if auth, ok := envCtx.Value(ContextBasicAuth).(BasicAuth); ok {
			localVarRequest.SetBasicAuth(auth.UserName, auth.Password)
		} else {
			reportError("Invalid KEYFACTOR_USERNAME and KEYFACTOR_PASSWORD credentials")
		}
